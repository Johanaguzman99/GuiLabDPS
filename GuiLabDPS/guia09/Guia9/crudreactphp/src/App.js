...
function App() {
  const baseUrl = "https://dps-rherrera.000webhostapp.com/Recursos/";
    ...
    .then(response=>{
      ...
      dataNueva.map(product=>{
        if(product.id===productoSeleccionado.id){
          product.nombre=productoSeleccionado.nombre;
          product.existencias=productoSeleccionado.existencias;
        }
      });
      setData(dataNueva);
